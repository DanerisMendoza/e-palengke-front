<template>
  <v-container>
    ...
    <v-row>
      <v-col cols="6">
        <v-card
          v-if="newOrder && isRunning && TRANSACTION.length != 0 && !hasDeliver"
        >
          <v-progress-linear
            color="red lighten-2"
            buffer-value="0"
          ...
          <v-card-text>
            ...
            <v-row v-for="(item, index) in TRANSACTION[0].orders" :key="index">
              <v-col>
                ...
                <v-divider></v-divider>
                <div
                  v-for="(item2, index2) in item.order_details"
                  :key="index2"
                >
...
export default {
  ...
  methods: {
    ...
    registeredLocation() {
      ...
      const targetItem = userDetails.find(
    ...
    FIND_NEAR_BY() {
      ...
      this.$store
        .dispatch("FIND_ORDER_WITHIN_RADIUS", payload)
        .then(async (response) => {
            ...
            if (this.isRunning && !this.hasDeliver) {
              ...
              while (
                ...
                if (this.isRunning && this.countdown <= 0) {
                  if (!this.hasDeliver) {
                    this.declinedTransactions.push(
                      this.TRANSACTION[0].transaction_id
                    );
                    this.$store.commit("ORDER_STORE_LAT_LNG", []);
                    this.$store.commit("TRANSACTION", []);
                    this.FIND_NEAR_BY();
    ...
    GET_IN_PROGRESS_TRANSACTION() {
      ...
      this.$store
        .dispatch("GET_IN_PROGRESS_TRANSACTION", payload)
        .then((response) => {
          if (response.length > 0) {
            this.$store.commit("TRANSACTION", response);
            this.$store.commit(
